...
export default function Auth({ onClose }) {
      ...
      if (isEmail) {
        if (mode === 'login' && !useOtpLogin) {
            ...
            if (!profileDoc.exists() || !profileDoc.data().full_name) {
                setProfile({ ...profile, email: emailLower });
        ...
        if (mode === 'signup' && !password) throw new Error("Please create a password for your new account.");
        ...
        if (mode === "signup") {
          ...
          // New phone user: no profile exists yet, so we cannot email them an OTP.

          // Go straight to the profile step so they can enter name + email.

          // Email OTP verification happens inside the profile step.

          setStep("profile");

          setSuccess("Please complete your profile to finish sign up.");

        } else {

          // Existing phone user: look up their email from the backend and send OTP there

          const phoneResponse = await fetch(`${API_BASE_URL}/auth/send-custom-email-otp`, {
          ...
          if (!phoneResponse.ok) throw new Error(phoneData.error || "Failed to send OTP. No email linked to this phone.");
          ...
          setSuccess(`OTP sent to your registered email for ${loginValue}`);
  ...
  const handleOtpVerify = async (e) => {
    ...
    try {
      ...
      if (isEmailOtpFlow) {
        // Custom OTP flow - works for both email and phone
        const rawId = identifier.trim();
        const isPhone = /^[+0-9\s-]{10,20}$/.test(rawId) && rawId.replace(/\D/g, '').length >= 10;
        const emailLower = isPhone ? null : rawId.toLowerCase();
        
        // Clean phone: ensure +91 and strip spaces
        let phoneVal = null;
        if (isPhone) {
          const digits = rawId.replace(/\D/g, '');
          phoneVal = `+91${digits.slice(-10)}`;
        }

        const response = await fetch(`${API_BASE_URL}/auth/verify-custom-email-otp`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ email: emailLower, phone: phoneVal, token: otpValue })
        });
        ...
        if (mode === 'signup' && password) { await updatePassword(user, password); }
      } else {
      ...
      if (!profileIsComplete) {
        if (isEmailOtpFlow && EMAIL_REGEX.test(identifier)) { setProfile({ ...profile, email: identifier.trim().toLowerCase() }); setEmailVerificationState({ code: '', sent: false, verified: true, loading: false }); }
        setStep("profile");
        setSuccess("Almost there! Complete your profile to continue.");
  ...
  const handleResendOtp = async () => {
    ...
    try {
      ...
      const isPhone = /^[+0-9]{10,14}$/.test(identifier.trim().replace(/\s/g, ''));

      const payload = isPhone ? { phone: identifier.trim() } : { email: identifier.trim().toLowerCase() };
  ...
  const handleVerifyEmail = async () => {
    ...
    const isValidEmail = EMAIL_REGEX.test(profile.email);
  ...
  const handleProfileSubmit = async (e) => {
    ...
    try {
      ...
      // Identify if this was a phone signup to preserve the phone number

      const cleanedId = identifier.replace(/\D/g, '').slice(-10);

      const isPhoneSignup = /^[0-9]{10}$/.test(cleanedId);

      // Sync full profile to Firestore
      ...
      await syncUserProfile(currentUser, {
        ...
        ...(isPhoneSignup ? { phone: identifier.trim() } : {})
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      // Do NOT call setLoading(true) before signInWithPopup.

      // A React setState re-render before window.open() breaks the user-gesture

      // chain and causes browsers to silently block the popup.
      ...
      // Sync profile in background - never block sign-in if sync fails

      syncUserProfile(result.user).catch(e => console.warn(e.message));
        ...
        {step === 'credentials' && (
          ...
          <form onSubmit={handleCredentialsSubmit} className="flex flex-col gap-4">
            ...
            >
              ...
              {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : <>{mode === 'login' ? (useOtpLogin || EMAIL_REGEX.test(identifier) ? 'Get OTP' : 'Sign In') : 'Sign Up'} <ArrowRight className="w-5 h-5" /></>}
          ...
          <form onSubmit={handleOtpVerify} className="flex flex-col gap-6">
            ...
            <div className="text-center">
              ...
              >
                ...
                Change {EMAIL_REGEX.test(identifier) ? 'Email' : 'Number'}
